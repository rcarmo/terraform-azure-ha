locals {
  ip_configuration_name = "${var.name}-config"
}
...
resource "azurerm_public_ip" "linux" {
  ...
  location                     = "${var.location}"
  resource_group_name          = "${var.resource_group_name}"
  sku                          = "Standard" # required for using Standard load balancers
  public_ip_address_allocation = "Static"   # required for Standard SKU
...
output "virtual_machine_ip_configuration" {
